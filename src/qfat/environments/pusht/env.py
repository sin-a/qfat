...
class DrawOptions(pymunk.SpaceDebugDrawOptions):
    ...
    def draw_circle(
        ...
        _ = to_pygame(circle_edge, self.surface)
...
class PushTEnv(gym.Env):
    ...
    def __init__(
        ...
        self.legacy = legacy
        self.use_success_thresh = use_success_thresh
    ...
    def reset(self):
        ...
        if state is None:
            state = np.array(
                [
                    self.rs.randint(50, 450),
                    self.rs.randint(50, 450),
                    self.rs.randint(100, 400),
                    self.rs.randint(100, 400),
                    self.rs.randn() * 2 * np.pi - np.pi,
                ]
            )
    ...
    def step(self, action):
        ...
        done = coverage > self.success_threshold if self.use_success_thresh else False
...
class PushTKeypointsEnv(PushTEnv):
    def __init__(
        ...
        super().__init__(
            ...
            use_success_thresh=use_success_thresh,
        )
        ws = self.window_size
...
class NormalizedPushTWrapper(Wrapper):
    def __init__(
        self,
        ...
        """
        Wraps the PushTKeypointsEnv environment to normalize only the first 20 states
        and unnormalize actions.
        ...
        super().__init__(
            PushTKeypointsEnv(
                reset_to_state=reset_to_state, use_success_thresh=use_success_thresh
            )
        )
    ...
    def step(self, action):
        ...
        obs = self.normalizer.normalize_state(np.array(obs[:20]))
        reward = self.coverage_arr[-1]
        return obs, reward, done, info

    def reset(self, **kwargs):
        obs = self.env.reset(**kwargs)
        obs = self.normalizer.normalize_state(obs[:20])
        return obs


class ImagePushTWrapper(Wrapper):
    def __init__(self, stats_path: str = str(PUSHT_DATA_PATH / "data_stats_img.json")):
        """
        Wraps the PushTKeypointsEnv environment, normalizes the actions and returns the resnet embeddings
            of the rendered state as an observation.

        Args:
            stats_path (str): Path to the JSON file containing the saved normalization statistics.
        """
        super().__init__(PushTKeypointsEnv())
    ...
    def step(self, action) -> Tuple[torch.Tensor, float, bool, Dict[str, Any]]:
        unnormalized_action = self.normalizer.unnormalize_action(action)
        _, reward, done, info = self.env.step(unnormalized_action.squeeze())
