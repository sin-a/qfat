...
class URRobot(object):
    ...
    def movej(
        self, joints, acc=0.1, vel=0.05, wait=True, relative=False, threshold=None
    ):
        """
        move in joint space
        """
    ...
    def movels(
        ...
        """
        return self.movexs(
            "movel", pose_list, acc, vel, radius, wait, threshold=threshold
        )
    ...
    def movexs(
        ...
        for idx, pose in enumerate(pose_list):
            prog += (
                self._format_move(
                    command, pose, acc, vel[idx], radius[idx], prefix=prefix
                )
                + "\n"
            )
        prog += end
