...
class RobotiqGripper:
    ...
    def _set_vars(self, var_dict):
        ...
        for variable, value in var_dict.items():
            ...
            cmd += " {} {}".format(
                variable, str(value)
            )  # for compatibility with lower versions of python
    ...
    def _get_var(self, variable):
        ...
        if var_name != variable:
            ...
            raise ValueError(
                "Unexpected response {} ({}): does not match '{}'".format(
                    data, data.decode(self.ENCODING), variable
                )
            )  # for compatibility with lower versions of python
        value = int(value_str)
        return value
    ...
    def auto_calibrate(self, log=True):
        ...
        if RobotiqGripper.ObjectStatus(status) != RobotiqGripper.ObjectStatus.AT_DEST:
            ...
            raise RuntimeError(
                "Calibration failed opening to start: {}".format(str(status))
            )  # for compatibility with lower versions of python
        ...
        if RobotiqGripper.ObjectStatus(status) != RobotiqGripper.ObjectStatus.AT_DEST:
            ...
            raise RuntimeError(
                "Calibration failed because of an object: {}".format(str(status))
            )  # for compatibility with lower versions of python
        ...
        if log:
            ...
            print(
                "Gripper auto-calibrated to [{}, {}]".format(
                    self.get_min_position(), self.get_max_position()
                )
            )  # for compatibility with lower versions of python
