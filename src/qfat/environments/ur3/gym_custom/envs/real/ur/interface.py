...
COMMAND_LIMITS = {
    "movej": [
        np.array([-2 * np.pi, -2 * np.pi, -np.pi, -2 * np.pi, -2 * np.pi, -np.inf]),
        np.array([2 * np.pi, 2 * np.pi, np.pi, 2 * np.pi, 2 * np.pi, np.inf]),
    ],  # [rad]
    "speedj": [
        np.array([-np.pi, -np.pi, -np.pi, -2 * np.pi, -2 * np.pi, -2 * np.pi]),
        np.array([np.pi, np.pi, np.pi, 2 * np.pi, 2 * np.pi, 2 * np.pi]),
    ],  # [rad/s]
...
class URScriptInterface(object):
    ...
    def log(self, msg, level="INFO"):
        ...
        if level == "INFO":
            self.__logger.info("[%s] " % (self.alias) + msg)
        elif level == "DEBUG":
            self.__logger.debug("[%s] " % (self.alias) + msg)
        elif level == "ERROR":
            self.__logger.error("[%s] " % (self.alias) + msg)
        else:
            pass
    ...
    def move_gripper_velocity(self, gd):  # currently, just binary action [open, close]
        ...
        warnings.warn(
            colorize(
                ...
                % (
                    "WARNING",
                    "Use move_gripper_position instead of this move_gripper_velocity",
                ),
                "yellow",
            )
        )
    ...
    def move_gripper_force(self, gf):  # currently, just binary action [open, close]
        ...
        warnings.warn(
            colorize(
                ...
                % (
                    "WARNING",
                    "Use move_gripper_position instead of this move_gripper_force",
                ),
                "yellow",
            )
        )
