...
import gym_custom
from gym_custom.envs.real.ur.interface import (
    URScriptInterface,
    convert_action_to_space,
...
class DualUR3RealEnv(gym_custom.Env):
    ...
    def __init__(self, host_ip_right, host_ip_left, rate):
        ...
        if self.ENABLE_COLLISION_CHECKER:
            ...
            assert mujoco_py_version == "1.50.1.68", (
                "mujoco-py version 1.50.1.68 required! got %s instead"
                % (mujoco_py_version)
            )
            try:
                pass
    ...
    def _define_class_variables(self):
        ...
        path_to_pkl = os.path.join(
            os.path.dirname(__file__), "ur/dual_ur3_kinematics_params.pkl"
        )
        if os.path.isfile(path_to_pkl):
            kinematics_params_from_pkl = pickle.load(open(path_to_pkl, "rb"))
            self.kinematics_params["T_wb_right"] = kinematics_params_from_pkl[
                "T_wb_right"
            ]
            self.kinematics_params["T_wb_left"] = kinematics_params_from_pkl[
                "T_wb_left"
            ]
        else:
            raise FileNotFoundError(
                "No such file: %s. Run MuJoCo-based simulated environment to generate file."
                % (path_to_pkl)
            )
    ...
    def inverse_kinematics_ee(
        ...
        if arm == "right":
            if type(q_init).__name__ == "ndarray":
                ...
                q = self.interface_right.get_joint_positions()
            elif q_init == "zero":
            ...
            if type(q_init).__name__ == "ndarray":
                ...
                q = self.interface_left.get_joint_positions()
            elif q_init == "zero":
                q = np.zeros([self.ur3_nqpos])
            else:
                raise ValueError(
    ...
    def set_initial_joint_pos(self, q=None):
        if q is None:
            ...
            self._init_qpos = np.concatenate(
                [
                    self.interface_right.get_joint_positions(),
                    self.interface_left.get_joint_positions(),
                ]
            ).ravel()
        else:
            assert q.shape[0] == 12
    ...
    def set_initial_joint_vel(self, qd=None):
        if qd is None:
            ...
            self._init_qvel = np.concatenate(
                [
                    self.interface_right.get_joint_speeds(),
                    self.interface_left.get_joint_speeds(),
                ]
            ).ravel()
        else:
    ...
    def set_initial_gripper_pos(self, g=None):
        if g is None:
            ...
            self._init_gripperpos = np.concatenate(
                [
                    self.interface_right.get_gripper_position(),
                    self.interface_left.get_gripper_position(),
                ]
            ).ravel()
        else:
            assert g.shape[0] == 2
    ...
    def set_initial_gripper_vel(self, gd=None):
        if gd is None:
            ...
            self._init_gripperpos = np.concatenate(
                [
                    self.interface_right.get_gripper_speed(),
                    self.interface_left.get_gripper_speed(),
                ]
            ).ravel()
        else:
            assert gd.shape[0] == 2
    ...
    def step(self, action, wait=False):
        ...
        self.run_before_rate_sleep_return = (
            self._run_before_rate_sleep_func()
        )  # run _run_before_rate_sleep_func
        ...
        if lag_occurred:
            warnings.warn(
                "Desired rate of %dHz is not satisfied! (current rate: %dHz)"
                % (self.rate._freq, 1 / (self.rate._actual_cycle_time))
            )
        controller_error = lambda stats: np.any(
            [
                (stat.safety.StoppedDueToSafety) or (not stat.robot.PowerOn)
                for stat in stats
            ]
        )
        if not TEMP_DISABLE_LEFT_ARM:
            if controller_error(
                [
                    self.interface_right.get_controller_status(),
                    self.interface_left.get_controller_status(),
                ]
            ):
    ...
    def reset(self):
        # TODO: Send commands to both arms simultaneously?
        self.interface_right.stopj(
            a=5, wait=True
        )  # prevent protecive stop(invalid setpoints: sudden stop) error
        if not TEMP_DISABLE_LEFT_ARM:
            self.interface_left.stopj(
                a=5, wait=True
            )  # prevent protecive stop(invalid setpoints: sudden stop) error
        controller_error = lambda stats: np.any(
            [
    ...
    def _recover_from_controller_error(self):
        ...
        robot_right = [
            attr
            for attr in dir(status_right.robot)
            if getattr(status_right.robot, attr) == True
        ]
        safety_right = [
            attr
            for attr in dir(status_right.safety)
            if getattr(status_right.safety, attr) == True
        ]
        robot_left = [
            attr
            for attr in dir(status_left.robot)
            if getattr(status_left.robot, attr) == True
        ]
        safety_left = [
            attr
            for attr in dir(status_left.safety)
            if getattr(status_left.safety, attr) == True
        ]
        status_info = {
            "real_env": True,
        ...
        if (not right_reset_done) or (not left_reset_done):
            while (not right_reset_done) or (not left_reset_done):
                ...
                print(
                    "ERR_FLAGS: \r\n right - %s, %s \r\n left - %s, %s"
                    % (robot_right, safety_right, robot_left, safety_left)
                )
                # beepy.beep('error')
                if (
                    prompt_yes_or_no(
                        "Press 'Y' after manual reset to proceed. Press 'n' to terminate program."
                    )
                    is False
                ):
        ...
        while not movej_success:
            try:
                ...
                for _ in range(2):
                    ...
                    movej_success = np.linalg.norm(
                        obs_dict["right"]["qpos"] - self._init_qpos[:6], np.inf
                    ) < np.deg2rad(3)
                    if movej_success:
                        break
                    time.sleep(0.1)
                ...
                if not movej_success:
                    ...
                    if (
                        prompt_yes_or_no(
                            "Press 'Y' to resend movej command. Press 'n' to terminate program."
                        )
                        is False
                    ):
                        print("exiting program!")
                ...
                if (
                    prompt_yes_or_no(
                        "Press 'Y' after untangling robot arms. Press 'n' to terminate program."
                    )
                    is False
                ):
                    print("exiting program!")
    ...
    def get_obs_dict(self, wait=False):
        _, curr_right_pos, _ = self.forward_kinematics_ee(
            self.interface_right.get_joint_positions(wait=wait), "right"
        )
        _, curr_left_pos, _ = self.forward_kinematics_ee(
            self.interface_left.get_joint_positions(wait=wait), "left"
        )
        return {
            "right": {
                ...
                "qpos_sine": np.sin(
                    self.interface_right.get_joint_positions(wait=wait)
                ),
                "qpos_cosine": np.cos(
                    self.interface_right.get_joint_positions(wait=wait)
                ),
            ...
            "left": {
                ...
                "qpos_cosine": np.cos(
                    self.interface_left.get_joint_positions(wait=wait)
                ),
    ...
    def _set_action_space():
        return convert_action_to_space(
            {"right": COMMAND_LIMITS, "left": COMMAND_LIMITS}
        )
...
def servoj_speedj_example(host_ip_right, host_ip_left, rate):
    real_env = DualUR3RealEnv(
        host_ip_right=host_ip_right, host_ip_left=host_ip_left, rate=rate
    )
    ...
    if (
        prompt_yes_or_no(
            "current qpos is \r\n right: %s deg\r\n left: %s deg\r\n?"
            % (np.rad2deg(real_env._init_qpos[:6]), np.rad2deg(real_env._init_qpos[6:]))
        )
        is False
    ):
        print("exiting program!")
    ...
    init_qpos_right, init_qpos_left = (
        obs_dict["right"]["qpos"],
        obs_dict["left"]["qpos"],
    )
    ...
    waypoints_qpos_right = np.linspace(
        init_qpos_right, goal_qpos_right, rate * 2, axis=0
    )
    ...
    if (
        prompt_yes_or_no(
            "servoj to \r\n right: %s deg\r\n left: %s deg\r\n?"
            % (np.rad2deg(goal_qpos_right), np.rad2deg(goal_qpos_left))
        )
        is False
    ):
        print("exiting program!")
    ...
    start = time.time()
    for n, (waypoint_right, waypoint_left) in enumerate(
        zip(waypoints_qpos_right[1:, :], waypoints_qpos_left[1:, :])
    ):
        real_env.step(
            {
                "right": {
    ...
    curr_qpos_right, curr_qpos_left = (
        curr_obs_dict["right"]["qpos"],
        curr_obs_dict["left"]["qpos"],
    )
    print(
        "current - goal qpos is \r\n right: %s deg\r\n left: %s deg"
        % (
            np.rad2deg(curr_qpos_right - goal_qpos_right),
            np.rad2deg(curr_qpos_left - goal_qpos_left),
        )
    )
    time.sleep(5)
    ...
    real_env.step(
        {
            "right": {"movej": {"q": waypoints_qpos_right[0, :]}},
            "left": {"movej": {"q": waypoints_qpos_left[0, :]}},
        }
    )
    print("done!")
    ...
    start = time.time()
    for n, (waypoint_right, waypoint_left) in enumerate(
        zip(waypoints_qvel_right, waypoints_qvel_left)
    ):
        real_env.step(
            {
                "right": {
...
if __name__ == "__main__":
    servoj_speedj_example(
        host_ip_right="192.168.5.102", host_ip_left="192.168.5.101", rate=10
    )
    pass
