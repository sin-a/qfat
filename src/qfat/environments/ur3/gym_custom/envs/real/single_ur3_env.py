...
class SingleUR3RealEnv(gym_custom.Env):
    ...
    def _define_collision_checker_variables(self):
        from gym_custom.envs.custom.single_ur3_env import SingleUR3Env

        self.collision_env = SingleUR3Env()
    ...
    def step(self, action, wait=False):
        ...
        if lag_occurred:
            ...
            warnings.warn(
                f"Desired rate of {self.rate._freq}Hz is not satisfied! (current rate: {1 / self.rate._actual_cycle_time}Hz)"
            )
    ...
    def get_obs_dict(self, wait=False):
        _, curr_pos, _ = self.forward_kinematics_ee(
    ...
    @staticmethod
    def _nparray_to_dict(
        obs_nparray,
    ):  # ee-Goal-conditioned setting (jonghae) + sine, cosine
        return {
            "right": {
                "curr_pos": obs_nparray[0:3],
                "curr_pos_block": obs_nparray[3:6],
...
## Examples
def servoj_speedj_example(host_ip_right, rate):
    real_env = SingleUR3RealEnv(host_ip_right=host_ip_right, rate=rate)
    real_env.set_initial_joint_pos("current")
    real_env.set_initial_gripper_pos("current")
    if (
        prompt_yes_or_no(
            "current qpos is \r\n right: %s deg?\r\n"
            % (np.rad2deg(real_env._init_qpos[:6]))
        )
        is False
    ):
        print("exiting program!")
    ...
    init_qpos_right = obs_dict["right"]["qpos"]
    goal_qpos_right = init_qpos_right.copy()
    ...
    print("close")
    real_env.step({"right": {"close_gripper": {}}})
    time.sleep(3.0)
    print("open")
    real_env.step({"right": {"open_gripper": {}}})
    time.sleep(3.0)
    print("close")
    real_env.step({"right": {"close_gripper": {}}})
    time.sleep(5.0)
    ...
    start = time.time()
    for n, (waypoint_right) in enumerate(zip(waypoints_qpos_right[1:, :])):
        real_env.step(
            {
                "right": {
                    "servoj": {
    ...
    curr_qpos_right = curr_obs_dict["right"]["qpos"]
    print(
    ...
    real_env.step({"right": {"movej": {"q": waypoints_qpos_right[0, :]}}})
    print("done!")
    ...
    start = time.time()
    for n, (waypoint_right) in enumerate(zip(waypoints_qvel_right)):
        real_env.step(
            {
                "right": {
                    "speedj": {
...
if __name__ == "__main__":
    servoj_speedj_example(host_ip_right="192.168.5.102", rate=10)
    pass
