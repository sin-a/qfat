...
class UR3RealEnv(gym_custom.Env):
    ...
    def set_initial_joint_vel(self, qd=None):
        if qd is None:
            ...
            self._init_qvel = self.interface.get_joint_speeds()
        else:
            assert qd.shape[0] == 6
            self._init_qvel = qd
    ...
    def set_initial_gripper_pos(self, g=None):
        if g is None:
            ...
            self._init_gripperpos = self.interface.get_gripper_position()
        else:
            assert g.shape[0] == 1
    ...
    def set_initial_gripper_vel(self, gd=None):
        if gd is None:
            ...
            self._init_grippervel = self.interface.get_gripper_position()
        else:
            assert gd.shape[0] == 1
    ...
    def step(self, action):
        ...
        controller_error = lambda status: (status.safety.StoppedDueToSafety) or (
            not status.robot.PowerOn
        )
    ...
    def _recover_from_controller_error(self):
        ...
        robot_status = [
            attr for attr in dir(status.robot) if getattr(status.robot, attr) == True
        ]
        safety_status = [
            attr for attr in dir(status.safety) if getattr(status.safety, attr) == True
        ]
        status_info = {"real_env": True, "error_flags": robot_status + safety_status}
        ...
        if not reset_done:
            while not reset_done:
                ...
                robot_status = [
                    attr
                    for attr in dir(status.robot)
                    if getattr(status.robot, attr) == True
                ]
                safety_status = [
                    attr
                    for attr in dir(status.safety)
                    if getattr(status.safety, attr) == True
                ]
...
def servoj_speedj_example(host_ip, rate):
    ...
    if (
        prompt_yes_or_no("current qpos is %s deg?" % (np.rad2deg(real_env._init_qpos)))
        is False
    ):
        print("exiting program!")
        sys.exit()
...
def sanity_check(host_ip):
    ...
    robot = URBasic.urScriptExt.UrScriptExt(
        host=host_ip, robotModel=robotModel, **gripper_kwargs
    )
    robot.reset_error()
    ...
    print(
        "curr_pos = get_actual_joint_positions() # %s deg" % (str(np.rad2deg(curr_pos)))
    )
    time.sleep(1)
