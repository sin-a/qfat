...
class URScriptWrapper(ActionWrapper):
    ...
    gripper_command_types = [
        "move_gripper_position",
        "move_gripper_velocity",
        "move_gripper_force",
    ]
    ...
    def __init__(
        self, env, PID_gains, ur3_scale_factor, gripper_scale_factor, _env_getters=None
    ):
        super().__init__(env)
        ...
        if _env_getters is None:
            self._env_getters = {
                ...
                "gpos": lambda: np.array(
                    [self.env._get_gripper_qpos()[2], self.env._get_gripper_qpos()[7]]
                ),
                "gvel": lambda: np.array(
                    [self.env._get_gripper_qvel()[2], self.env._get_gripper_qvel()[7]]
                ),
                "gbias": lambda: np.array(
                    [self.env._get_gripper_bias()[2], self.env._get_gripper_bias()[7]]
                ),
                "dt": self.env.dt,
            }
        else:
        ...
        self._initialize_attributes(
            copy.deepcopy(PID_gains),
            ur3_scale_factor.copy(),
            gripper_scale_factor.copy(),
        )
    ...
    def _initialize_attributes(self, PID_gains, ur3_scale_factor, gripper_scale_factor):
        ...
        self.ndof, self.ngripperdof = (
            ur3_scale_factor.shape[0],
            gripper_scale_factor.shape[0],
        )
        ...
        self.ur3_err_integ, self.gripper_err_integ = (
            np.zeros([self.ndof]),
            np.zeros([2 * self.ngripperdof]),
        )
    ...
    def action(self, ur_command):
        if not all(
            [
                command_type in self.command_type_list
                for command_type in ur_command.keys()
            ]
        ):
            raise ValueError("Invalid command type!")
        self._clear_integration_term(
            ur_command, force=False
        )  # clear integration term if command type has changed

        # generate actions
        actions = {}
        for command_type, command_val in ur_command.items():
            action, entity = getattr(self, command_type)(**command_val)
            if entity in actions.keys():
                raise ValueError("Multiple commands for a single entity!")
            else:
    ...
    def servoj(
        self, q, t=None, lookahead_time=None, gain=None, wait=None
    ):  # we only use q here
        """
        ...
        self.ur3_err_integ = np.clip(
            self.ur3_err_integ + err * self._env_getters["dt"], -1, 1
        )
        ...
        constraint = np.clip(
            constraint, -0.50 * self.ur3_torque_limit, 0.50 * self.ur3_torque_limit
        )
        ...
        PID_control = self.ur3_scale_factor * (
            self.servoj_gains["P"] * err
            + self.servoj_gains["I"] * self.ur3_err_integ
            + self.servoj_gains["D"] * err_dot
        )
    ...
    def speedj(self, qd, a=None, t=None, wait=None):  # we only use qd here
        ...
        self.ur3_err_integ = np.clip(
            self.ur3_err_integ + err * self._env_getters["dt"], -0.02, 0.02
        )
        ...
        PI_control = self.ur3_scale_factor * (
            self.speedj_gains["P"] * err + self.speedj_gains["I"] * self.ur3_err_integ
        )
    ...
    def move_gripper_position(self, g):
        ...
        action = (
            self.gripper_scale_factor * err + self._env_getters["gbias"]()
        )  # P control
        return action, "gripper"
...
class URScriptWrapper_DualUR3(ActionWrapper):
    def __init__(self, env, PID_gains, ur3_scale_factor, gripper_scale_factor):
        ...
        wrapper_right_env_getters = {
            ...
            "gpos": lambda: np.array(
                [self.env.get_gripper_qpos()[2], self.env.get_gripper_qpos()[7]]
            ),
            "gvel": lambda: np.array(
                [self.env.get_gripper_qvel()[2], self.env.get_gripper_qvel()[7]]
            ),
            "gbias": lambda: np.array(
                [self.env.get_gripper_bias()[2], self.env.get_gripper_bias()[7]]
            ),
            "dt": env.dt,
        }
        wrapper_left_env_getters = {
            ...
            "qconstraint": lambda: env.get_ur3_constraint()[env.ur3_nqvel :],
            "gpos": lambda: np.array(
                [self.env.get_gripper_qpos()[12], self.env.get_gripper_qpos()[17]]
            ),
            "gvel": lambda: np.array(
                [self.env.get_gripper_qvel()[12], self.env.get_gripper_qvel()[17]]
            ),
            "gbias": lambda: np.array(
                [self.env.get_gripper_bias()[12], self.env.get_gripper_bias()[17]]
            ),
            "dt": env.dt,
        }
        self.wrapper_right = URScriptWrapper(
            env,
            PID_gains,
            ur3_scale_factor,
            gripper_scale_factor,
            _env_getters=wrapper_right_env_getters,
        )
        self.wrapper_left = URScriptWrapper(
            env,
            PID_gains,
            ur3_scale_factor,
            gripper_scale_factor,
            _env_getters=wrapper_left_env_getters,
        )
    ...
    def action(self, ur_command):  # note that ur_command is dictionary
        ...
        right_ur3_action, right_gripper_action = (
            right_action[: self.wrapper_right.ndof],
            right_action[self.wrapper_right.ndof :],
        )  # ndof, 2*ngripperdof
        left_ur3_action, left_gripper_action = (
            left_action[: self.wrapper_left.ndof],
            left_action[self.wrapper_left.ndof :],
        )  # ndof, 2*ngripperdof
