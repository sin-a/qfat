...
class SingleUR3XYEnv(MujocoEnv, utils.EzPickle):
    ...
    def _check_model_parameter_dimensions(self):
        ...
        assert (
            self.ur3_nact + self.gripper_nact == self.model.nu
    ...
    def _get_gripper_qpos(self):
        return np.concatenate(
            [self.sim.data.qpos[self.ur3_nqpos : self.ur3_nqpos + self.gripper_nqpos]]
        ).ravel()
    ...
    def _get_gripper_qvel(self):
        return np.concatenate(
            [self.sim.data.qvel[self.ur3_nqvel : self.ur3_nqvel + self.gripper_nqvel]]
        ).ravel()
    ...
    def reset_model(self):
        ...
        random_point = np.array(
            [
                center_x + random_radius * np.cos(random_angle),
                center_y + random_radius * np.sin(random_angle),
            ]
        )
