...
class SingleUR3XYCOMBEnv(MujocoEnv, utils.EzPickle):
    ...
    mujoco_xml_full_path = os.path.join(
        os.path.dirname(__file__), "assets/ur3/single_ur3_base.xml"
    )
    ...
    objects_nqpos = [
        ...
        7,
        7,
    ]  # there is 4 objects on the table, each object has qpos = (3trans + 4quat)
    objects_nqvel = [
        6,
        6,
        6,
        6,
    ]  # there is 4 objects on the table, each object has qpos = (3trans + 3rota)
    # action
    ...
    def _check_model_parameter_dimensions(self):
        """overridable method"""
        assert (
            self.ur3_nqpos + self.gripper_nqpos + sum(self.objects_nqpos)
            == self.model.nq
        ), "Number of qpos elements mismatch"
        assert (
            self.ur3_nqvel + self.gripper_nqvel + sum(self.objects_nqvel)
            == self.model.nv
        ), "Number of qvel elements mismatch"
        assert self.ur3_nact + self.gripper_nact == self.model.nu, (
    ...
    def _define_class_variables(self):
        ...
        self.init_qpos[0 : self.ur3_nqpos] = np.array(
            [1.19233047, -1.51248855, 1.62547755, -2.04504378, 0.85489129, -0.01958713]
        )  # for (0.45, -0.325)
        ...
        self.kinematics_params["T_wb_right"][0:3, 0:3] = (
            self.sim.body("right_arm_rotz").xmat.reshape([3, 3]).copy()
        )
        self.kinematics_params["T_wb_right"][0:3, 3] = self.sim.body(
            "right_arm_rotz"
        ).xpos.copy()
    ...
    def get_body_se3(self, body_name):
        R = self.sim.get_body_xmat(body_name).reshape([3, 3]).copy()
        p = self.sim.get_body_xpos(body_name).copy()
        T = np.eye(4)
        T[0:3, 0:3] = R
        T[0:3, 3] = p
    ...
    def _get_ur3_qpos(self):
        return np.concatenate([self.sim.qpos[0 : self.ur3_nqpos]]).ravel()

    def _get_gripper_qpos(self):
        return np.concatenate(
            [self.sim.qpos[self.ur3_nqpos : self.ur3_nqpos + self.gripper_nqpos]]
        ).ravel()

    def _get_ur3_qvel(self):
        return np.concatenate([self.sim.qvel[0 : self.ur3_nqvel]]).ravel()

    def _get_gripper_qvel(self):
        return np.concatenate(
            [self.sim.qvel[self.ur3_nqvel : self.ur3_nqvel + self.gripper_nqvel]]
        ).ravel()

    def _get_ur3_bias(self):
        return np.concatenate([self.sim.qfrc_bias[0 : self.ur3_nqvel]]).ravel()

    def _get_gripper_bias(self):
        return np.concatenate(
            [self.sim.qfrc_bias[self.ur3_nqvel : self.ur3_nqvel + self.gripper_nqvel]]
        ).ravel()

    def _get_ur3_constraint(self):
        return np.concatenate([self.sim.qfrc_constraint[0 : self.ur3_nqvel]]).ravel()

    def _get_ur3_actuator(self):
        return np.concatenate([self.sim.qfrc_actuator[0 : self.ur3_nqvel]]).ravel()
    ...
    def _get_obs(self):
        ...
        return np.concatenate(
            [
                self.curr_pos,
                self.curr_pos_block,
                self.curr_pos_block2,
                np.sin(self._get_ur3_qpos()),
                np.cos(self._get_ur3_qpos()),
                self._get_gripper_qpos(),
                self._get_gripper_qvel(),
            ]
        ).ravel()
    ...
    def step(self, a):
        ...
        # cube pos
        id_cube_6 = self.sim.model.geom("cube_6").id
        id_cube_7 = self.sim.model.geom("cube_7").id
        self.curr_pos_block = np.concatenate([self.sim.geom_xpos[id_cube_6][:2]])
        self.curr_pos_block2 = np.concatenate([self.sim.geom_xpos[id_cube_7][:2]])

        # gripper pos
        SO3, curr_pos, _ = self.forward_kinematics_ee(
        ...
        # reward_bound
        is_inside_bound = self.is_inside_bound(
            self.curr_pos[0], self.curr_pos[1], -0.1, -0.7, 0.75, 0.80
        )
    ...
    def reset_model(self):
        ...
        random_point_red = np.array(
            [
                center_x_red + random_radius_red * np.cos(random_angle_red),
                center_y_red + random_radius_red * np.sin(random_angle_red),
            ]
        )
        random_point_blue = np.array(
            [
                center_x_blue + random_radius_blue * np.cos(random_angle_blue),
                center_y_blue + random_radius_blue * np.sin(random_angle_blue),
            ]
        )
    ...
    def viewer_setup(self):
        ...
        v = self.viewer
        v.cam.trackbodyid = 0
        v.cam.distance = 10  # self.model.stat.extent
    ...
    def is_inside_bound(self, x, y, start_x, start_y, bound_width, bound_height):
        return (
            start_x <= x < start_x + bound_width
            and start_y <= y < start_y + bound_height
        )
