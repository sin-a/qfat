...
class PracticeEnv1(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, actuator_type):
        ...
        fullpath = os.path.join(
            os.path.dirname(__file__), "assets", "practice", xml_filename
        )
...
class PracticeEnv2(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self):
        fullpath = os.path.join(
            os.path.dirname(__file__), "assets", "practice", "practice2.xml"
        )
...
class PracticeEnv3(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self):
        fullpath = os.path.join(
            os.path.dirname(__file__), "assets", "practice", "practice3.xml"
        )
        MujocoEnv.__init__(self, fullpath, 1)
        utils.EzPickle.__init__(self)
...
class PracticeEnv4(MujocoEnv, utils.EzPickle):
    def __init__(self):
        fullpath = os.path.join(
            os.path.dirname(__file__), "assets", "practice", "practice4_kinematics.xml"
        )
        MujocoEnv.__init__(self, fullpath, 1)
        utils.EzPickle.__init__(self)
    ...
    def reset_model(self):
        self.set_state(
            self.init_qpos
            + self.np_random.uniform(low=-0.1, high=0.1, size=self.model.nq) * 0.0,
            self.init_qvel + self.np_random.randn(self.model.nv) * 0.1 * 0.0,
        )
        return self._get_obs()
