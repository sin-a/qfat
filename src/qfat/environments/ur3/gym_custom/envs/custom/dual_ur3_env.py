...
class DualUR3Env(MujocoEnv, utils.EzPickle):
    ...
    mujoco_xml_full_path = os.path.join(
        os.path.dirname(__file__), "assets/ur3/dual_ur3_base.xml"
    )
    ...
    curr_pos_block = np.array([1, 1, 1, 1])
    ...
    def step(self, a):
        ...
        # curr pos block
        id_cube_1 = self.sim.model.geom_name2id("cube_1")  # right
        id_cube_4 = self.sim.model.geom_name2id("cube_4")  # left
        self.curr_pos_block = np.concatenate(
            [
                self.sim.data.geom_xpos[id_cube_1][:2],
                self.sim.data.geom_xpos[id_cube_4][:2],
            ]
        )
