...
from gym_custom import error, version, logger
import os
import json
import numpy as np
from gym_custom.wrappers.monitoring import stats_recorder, video_recorder
from gym_custom.utils import atomic_write, closer
from gym_custom.utils.json_utils import json_encode_np
...
class Monitor(Wrapper):
    ...
    def _start(
        ...
        if self.env.spec is None:
            logger.warn(
                "Trying to monitor an environment which has no 'spec' set. This usually means you did not create it via 'gym_custom.make', and is recommended only for advanced users."
            )
            env_id = "(unknown)"
        else:
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.""".format(
    ...
    def close(self):
        ...
        logger.info(
            """Finished writing results. You can upload them to the scoreboard via gym_custom.upload(%r)""",
            self.directory,
        )
    ...
    def _env_info(self):
        env_info = {
            "gym_version": version.__version__,
        }
        if self.env.spec:
