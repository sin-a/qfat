...
import gym_custom
from gym_custom.wrappers import TransformReward
...
def test_transform_reward(env_id):
    ...
    for scale in scales:
        ...
        wrapped_env = TransformReward(gym_custom.make(env_id), lambda r: scale * r)
    ...
    wrapped_env = TransformReward(
        gym_custom.make(env_id), lambda r: np.clip(r, min_r, max_r)
    )
    action = env.action_space.sample()
    ...
    # use case #3: sign
    env = gym_custom.make(env_id)
    wrapped_env = TransformReward(gym_custom.make(env_id), lambda r: np.sign(r))
