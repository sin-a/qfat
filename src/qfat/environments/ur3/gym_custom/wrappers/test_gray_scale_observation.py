...
import gym_custom
from gym_custom.wrappers import GrayScaleObservation
from gym_custom.wrappers import AtariPreprocessing
...
def test_gray_scale_observation(env_id, keep_dim):
    gray_env = AtariPreprocessing(
        gym_custom.make(env_id), screen_size=84, grayscale_obs=True
    )
    rgb_env = AtariPreprocessing(
        gym_custom.make(env_id), screen_size=84, grayscale_obs=False
    )
