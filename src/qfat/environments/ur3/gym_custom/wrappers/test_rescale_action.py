...
import gym_custom
from gym_custom.wrappers import RescaleAction


def test_rescale_action():
    env = gym_custom.make("CartPole-v1")
    with pytest.raises(AssertionError):
        env = RescaleAction(env, -1, 1)
    del env

    env = gym_custom.make("Pendulum-v0")
    wrapped_env = RescaleAction(gym_custom.make("Pendulum-v0"), -1, 1)
